...
from app.models.streetview import StreetViewGraph, StreetViewGraphCreate, StreetViewNodeUpdate
...
@router.patch(
    ...
    response_description="Bestätigung des Updates",
    responses={
        200: {"content": {"application/json": {"example": {"status": "updated", "node_id": "G2-EG-G01"}}}},
        404: {"description": "Gebäude-Graph oder Node nicht gefunden"},
        500: {"description": "Datenbankfehler"},
    },
)
async def patch_node(
    building_id: str,
    node_id: str,
    body: StreetViewNodeUpdate,
) -> dict[str, str]:
    """Aktualisiert einen einzelnen Node im Gebäude-Graphen ohne den Rest zu überschreiben.

    Nur Felder die im Body gesetzt sind werden geändert.
    Nützlich um nach und nach Fotos, exits oder nearby_rooms einzupflegen.
    """
    try:
        db = mongo_client.get_db()
        doc = db.streetview_graphs.find_one({"building_id": building_id})
        if not doc:
            raise HTTPException(status_code=404, detail=f"Kein Graph für Gebäude '{building_id}' gefunden")

        nodes: list[dict] = doc["graph"].get("nodes", [])
        node_index = next((i for i, n in enumerate(nodes) if n.get("id") == node_id), None)
        if node_index is None:
            raise HTTPException(status_code=404, detail=f"Node '{node_id}' nicht im Graph von '{building_id}' gefunden")
        ...
        logger.error(f"Error patching node {node_id} in building {building_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e)) from e
