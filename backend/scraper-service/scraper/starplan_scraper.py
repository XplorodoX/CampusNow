...
class StarplanScraper:
    ...
    def _scrape_rooms(self) -> list[dict[str, Any]]:
        ...
        for item in rooms_payload:
            ...
            # Try to infer building code from provided fields or the room label
            bshort = item.get("buildingshortname") or item.get("buildingShortname")
            bid = item.get("buildingid") or item.get("buildingId")
            if not bshort and not bid:
                # Extract leading letters+digits from room label (e.g. 'G2 1.01' -> 'G2', 'AH -1.01' -> 'AH')
                m = re.match(r"^([A-Za-z]+\d*)", room_label.strip())
                if m:
            ...
            rooms.append(
                {
                    ...
                    "building_shortname": bshort,
                    "building_id": bid,
                    "capacity": item.get("capacity") or item.get("seats"),
